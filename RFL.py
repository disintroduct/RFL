...
import Signing_and_Verifying as sign
import LHH
...
def setupSEAL(w):
    ...
    w_enc = ts.ckks_tensor(context, w)
    w_enc.decrypt()
    return w_enc
            ...
            for j in range(len(set_variable1)): 
                ...
                for m in range(args.num_experiments):
                    ...
                    for iter in range(args.epochs):
                        ...
                        ### perturb 'w_local' ###
                        w_locals = setupSEAL(w_locals)
                        # w_locals = noise_add(args, noise_scale, w_locals)                        
                        
                        # sign and authentication
                        verify_list = []
                        for i in range(len(w_locals)):
                            pri_key, pub_key = LHH.generate_keypair()
                            (r,s) = sign.sign(str(w_locals[i]), private_key=pri_key, SHA=sha384)
                            Vcode = sign.verifying(str(w_locals[i]), pub_key, sha384)
                            if Vcode == 0:
...
if __name__ == '__main__':    
    ...
    args.set_privacy_budget = [10] # range(8,24,4)
    args.set_epochs = [50] # range(100,305,50)
